...
export const dataToTreeData = (
    ...
    for (let i = maxLayer; i > 0; --i) {
      x6Nodes.push({
        ...
        attrs: {
          ...
          label: {
            // text: romanize(i), //TODO 使用节点的name
            fill: "#666",
            fontSize: 48,
          },
        },
  ...
  dataCopy.forEach((node) => {
    ...
    const parentId = node.parentId; // 获取父节点的Id
    ...
    if (parentId) {
      ...
      if (parentNode.subTreeWidth === 0) {
        ...
        parentNode.subTreeWidth =
          parentNode.subTreeWidth + maxSubtreeWidth + nodeGutter; // 父节点的子树增加并且加上节点间距
      }
    }
  });
  ...
  idToX6Map[-1].x =
    parseInt(idToCalcNodesMap[-1].subTreeWidth / 2) - nodeWidth / 2; // 根树的x结点
  idToX6Map[-1].y = 200;
  ...
  fromRootToLeaf.forEach((node) => {
    ...
    const parentId = calcNode.parentId;
    const parentCalcNode = idToCalcNodesMap[parentId];
    const parentX6Node = idToX6Map[parentId];
    ...
    const subTreeStartX =
      parentX6Node.x +
      parseInt(calcNode.width / 2 - parentCalcNode.subTreeWidth / 2) +
      parentCalcNode.usedWidth;
    // 当前节点子树开始的y坐标: 父节点的 y 左边 + 层间距 + 节点高度
    const subTreeStartY = parentX6Node.y + layerGutter + nodeHeight;

    //
    const nodeX = parseInt(
      subTreeStartX + calcNode.subTreeWidth / 2 - calcNode.width / 2
    );
  ...
  return {
    nodes: [
      ...
      ...Object.keys(collapsableNodes).map((key) => ({
        ...collapsableNodes[key],
        id: `expand-${key}`,
      })),
    ],
    edges: x6Edges,
    size: {
      ...
      height:
        Math.max(...calcNodes.map((item) => item.layer)) *
          (nodeHeight + layerGutter) -
        layerGutter,
    },
  };
};
